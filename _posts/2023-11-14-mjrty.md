...
The MJRTY algorithm, proposed in $$[1]$$, is a popular majority voting algorithm that runs in linear time. In this post I'm not going to focus on the algorithm per se but rather on the constraints that are imposed on the algorithm. Specifically I'm going to offer an explanation for why one of the constraints on MJRTY can be removed (the authors mention this but offer no explanation). So read through the paper (<a href="/assets/mjrty/paper.pdf"><i>here</i></a>) and come back.  
